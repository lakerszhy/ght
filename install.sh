...
# This script installs ght on Linux and macOS.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/l-z-h/ght/main/scripts/install.sh | sh
#
# The script will:
# 1. Detect the user's OS and architecture.
# 2. Download the latest release of ght for their platform.
...
main() {
  ...
  latest_version_url=$(curl -sL -o /dev/null -w %{url_effective} https://github.com/lakerszhy/ght/releases/latest)
  latest_version=$(basename "$latest_version_url")

  if [ -z "$latest_version" ]; then
    echo -e "${red}❌ Failed to fetch the latest version of ght.${nc}"
    exit 1
  fi

  # Construct the download URL
  download_url="https://github.com/lakerszhy/ght/releases/download/${latest_version}/ght-${latest_version}-${os}_${arch}.tar.gz"

  echo -e "${green}⬇️  Downloading ght ${latest_version} for ${os}/${arch}...${nc}"
  ...
  if ! curl -L "$download_url" | tar -xz -C "$temp_dir"; then
    echo -e "${red}❌ Failed to download or extract ght. Please check the URL and your network connection.${nc}"
    exit 1
  fi
  ...
  archive_dir="ght-${latest_version}-${os}_${arch}"

  # Move the binary to the installation directory
  # Try /usr/local/bin first, then fallback to ~/.local/bin
  if [ -w /usr/local/bin ]; then
    install_dir="/usr/local/bin"
    echo -e "${yellow}Installing ght to ${install_dir}...${nc}"
    if mv "${temp_dir}/${archive_dir}/ght" "${install_dir}/"; then
      ...
      mkdir -p "${install_dir}"
      if ! mv "${temp_dir}/${archive_dir}/ght" "${install_dir}/"; then
        echo -e "${red}❌ Failed to install ght to both locations.${nc}"
        exit 1
      fi
    fi
  else
    ...
    mkdir -p "${install_dir}"
    if ! mv "${temp_dir}/${archive_dir}/ght" "${install_dir}/"; then
      echo -e "${red}❌ Failed to install ght.${nc}"
      exit 1
    fi
  fi
  ...
  echo -e "You can now run 'ght' to start the application."
}
